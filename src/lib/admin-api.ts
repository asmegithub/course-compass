...
export const getCertificateTemplates = async (): Promise<
  CertificateTemplate[]
> => {
  const data = await apiFetch<CertificateTemplate[]>(
...
export const createPermission = async (
  payload: Partial<RbacPermission>,
...
export const deleteCertificateTemplate = async (id: string): Promise<void> => {
  await apiFetch<void>(`/api/certificate-templates/${id}`, {
    method: "DELETE",
  });
};
...
export interface PaymentProof {
  ...
  order?: {
    id?: string;
    items?: { course?: { id?: string; title?: string } }[];
  };
  course?: { id?: string; title?: string };
...
export const approvePaymentProof = async (
  ...
  return apiFetch<PaymentProof>(`/api/payment-proofs/${proofId}/approve`, {
    method: "POST",
  });
};
...
export const getPaymentProofReceiptBlob = async (
  proofId: string,
): Promise<Blob> => {
...
export const getPendingReferralWithdrawals = async (): Promise<
  AdminReferralWithdrawalRequest[]
> => {
...
export const approveReferralWithdrawal = async (payload: {
  requestId: string;
  file?: File | null;
}): Promise<void> => {
  ...
  await apiFetch<void>(
    `/api/referral-balance/admin/withdrawals/${payload.requestId}/approve`,
    { method: "POST", body: form },
  );
};

export const rejectReferralWithdrawal = async (payload: {
  requestId: string;
  reason?: string;
}): Promise<void> => {
  await apiFetch<void>(
    `/api/referral-balance/admin/withdrawals/${payload.requestId}/reject`,
    {
      method: "POST",
...
export const getPendingInstructorPayoutRequests = async (): Promise<
  AdminPayoutRequest[]
> => {
  const data = await apiFetch<
    Array<{
  ...
  return (Array.isArray(data) ? data : []).map((r) => ({
    ...
    instructorName:
      [
        r.instructorProfile?.user?.firstName,
        r.instructorProfile?.user?.lastName,
      ]
        .filter(Boolean)
        .join(" ") || undefined,
...
export const approveInstructorPayoutRequest = async (payload: {
  requestId: string;
  file: File;
}): Promise<void> => {
  const form = new FormData();
  form.append("file", payload.file);
  await apiFetch<void>(`/api/instructor-payouts/approve/${payload.requestId}`, {
    method: "POST",
    body: form,
  });
};

export const rejectInstructorPayoutRequest = async (payload: {
  requestId: string;
  reason?: string;
}): Promise<void> => {
...
export const getPayoutMethodOptions = async (): Promise<
  PayoutMethodOption[]
> => {
  const data = await apiFetch<PayoutMethodOption[]>(
    "/api/payout-method-options",
  );
  return Array.isArray(data) ? data : [];
};

export const createPayoutMethodOption = async (
  payload: Partial<PayoutMethodOption>,
...
export const updatePayoutMethodOption = async (
  id: string,
  payload: Partial<PayoutMethodOption>,
): Promise<PayoutMethodOption> => {
...
export const deletePayoutMethodOption = async (id: string): Promise<void> => {
  await apiFetch<void>(`/api/payout-method-options/${id}`, {
    method: "DELETE",
  });
};
