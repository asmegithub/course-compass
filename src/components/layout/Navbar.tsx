...
const Navbar = () => {
  ...
  const userRole: 'GUEST' | 'STUDENT' | 'INSTRUCTOR' | 'ADMIN' = 'STUDENT';
        ...
        <nav className="hidden lg:flex items-center gap-6">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="start" className="w-56 bg-card">
              ...
              <DropdownMenuItem asChild>
                <Link to="/courses?category=languages">🌍 Languages</Link>
              </DropdownMenuItem>
              <DropdownMenuSeparator />
              <DropdownMenuItem asChild>
                <Link to="/categories" className="font-medium">View All Categories</Link>
              </DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
        </nav>
        ...
        <form onSubmit={handleSearch} className="flex-1 max-w-xl hidden md:flex">
          <div className="relative w-full">
            ...
            <Input
              ...
              className="w-full pl-10 pr-4 bg-muted/50 border-0 focus-visible:ring-1 focus-visible:ring-accent"
            />
          </div>
        </form>
          ...
          {isLoggedIn ? (
            <>
              ...
              <Button variant="ghost" size="icon" className="relative">
                ...
                <span className="absolute -top-1 -right-1 h-5 w-5 rounded-full bg-accent text-xs text-accent-foreground flex items-center justify-center font-semibold">
                  2
                </span>
              </Button>
              ...
              <Button variant="ghost" size="icon" className="relative">
                ...
                <span className="absolute -top-1 -right-1 h-5 w-5 rounded-full bg-destructive text-xs text-destructive-foreground flex items-center justify-center font-semibold">
                  3
                </span>
              </Button>
        ...
        <div className="container py-4 space-y-4 border-t border-border">
          ...
          <nav className="flex flex-col gap-2">
            ...
            {!isLoggedIn && (
              <Link 
                to="/auth" 
                className="px-4 py-2 text-sm font-medium text-muted-foreground hover:text-foreground hover:bg-muted rounded-lg transition-colors sm:hidden"
                onClick={() => setIsMenuOpen(false)}
              >
                Sign In
              </Link>
