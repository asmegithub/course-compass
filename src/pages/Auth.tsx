      ...
      <div className="flex-1 flex flex-col justify-center py-12 px-4 sm:px-6 lg:px-20 xl:px-24 bg-card">
        <div className="mx-auto w-full max-w-sm">
          ...
          <p className="mt-8 text-center text-sm text-muted-foreground">
            {mode === 'signin' ? (
              <>{t('auth.dontHaveAccount')}{' '}<button type="button" onClick={() => setMode('signup')} className="text-accent font-medium hover:underline">{t('auth.signUp')}</button></>
            ) : (
              <>{t('auth.alreadyHaveAccount')}{' '}<button type="button" onClick={() => setMode('signin')} className="text-accent font-medium hover:underline">{t('common.signIn')}</button></>
            )}
          </p>
        </div>
      </div>
