...
const InstructorStudents = () => {
  ...
  const instructorCourseIds = useMemo(() => {
    ...
    return new Set(courses.filter((course) => course.instructorId === user.id).map((course) => course.id));
  }, [courses, user?.id]);
  ...
  const stats = useMemo(() => {
    ...
    const totalStudents = new Set(instructorEnrollments.map((enrollment) => enrollment.studentId).filter(Boolean)).size;
    const completedCourses = instructorEnrollments.filter((enrollment) => enrollment.isCompleted || enrollment.progress >= 100).length;
    ...
    const avgCompletion = totalEnrolled > 0
      ? Math.round(instructorEnrollments.reduce((sum, enrollment) => sum + enrollment.progress, 0) / totalEnrolled)
      : 0;
    ...
    <DashboardLayout>
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {[
            { label: 'Total Enrolled', value: stats.totalEnrolled.toLocaleString(), icon: Users },
            { label: 'Active This Week', value: stats.activeThisWeek.toLocaleString(), icon: TrendingUp },
            { label: 'Completed Course', value: stats.completedCourses.toLocaleString(), icon: GraduationCap },
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            {isLoading && <p className="text-center text-muted-foreground py-8">Loading students...</p>}
            {isError && <p className="text-center text-destructive py-8">Failed to load student data.</p>}
            {!isLoading && !isError && filtered.map((student) => (
              <div key={student.id} className="flex flex-col sm:flex-row sm:items-center gap-4 p-4 rounded-lg border bg-muted/30 hover:bg-muted/60 transition-colors">
                <Avatar className="h-10 w-10">
                  ...
                  <AvatarFallback className="bg-primary/10 text-primary text-sm font-semibold">
                    {(student.studentName || 'S').split(' ').map((name) => name[0]).join('')}
                  </AvatarFallback>
                </Avatar>
                <div className="flex-1 min-w-0">
                  <p className="font-semibold text-sm">{student.studentName || 'Student'}</p>
                  <p className="text-xs text-muted-foreground">{student.studentEmail || 'No email'}</p>
                  <p className="text-xs text-muted-foreground mt-1">
                    <span className="font-medium text-foreground">{student.courseTitle || 'Course'}</span> · Enrolled {formatDate(student.enrolledAt)}
                  </p>
                </div>
                <div className="flex items-center gap-4">
                  <div className="text-right">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs font-medium w-8">{Math.round(student.progress)}%</span>
                    </div>
                    <p className="text-xs text-muted-foreground mt-1">Last active {formatRelativeTime(student.updatedAt)}</p>
                  </div>
                  <Button
                    variant="ghost"
                    size="icon"
