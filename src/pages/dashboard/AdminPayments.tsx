...
function formatStudentName(
  student: AdminPayment["student"] | PaymentProof["student"],
): string {
  if (!student) return "–";
...
function formatProofFor(proof: PaymentProof): string {
  if (proof.order?.items?.length) {
    const titles = proof.order.items
      .map((i) => i.course?.title)
      .filter(Boolean);
    return titles.length
      ? `Cart: ${titles.join(", ")}`
      : `Cart order ${proof.order.id ?? ""}`;
  }
...
const AdminPayments = () => {
  ...
  const rejectMutation = useMutation({
    mutationFn: ({ proofId, reason }: { proofId: string; reason?: string }) =>
      rejectPaymentProof(proofId, reason),
    onSuccess: () => {
      toast({
  ...
  const handleReject = (proof: PaymentProof) => {
    ...
    const reason = window.prompt(
      "Optional: add a rejection reason shown to the student.",
    );
  ...
  const filtered = useMemo(() => {
    return payments.filter((t) => {
      ...
      const courseStr =
        t.course?.title || t.course?.id || (t.order ? "Cart" : "") || "";
  ...
  const totalFees = useMemo(
    () =>
      filtered
        .filter((t) => t.status === "COMPLETED")
        .reduce((s, t) => s + (t.platformShare ?? 0), 0),
    [filtered],
  );
  ...
  return (
    <DashboardLayout>
      <div className="space-y-6">
          ...
          <CardContent className="space-y-4 pt-6">
            <div className="flex items-center justify-between gap-2">
              <div>
                <p className="text-sm font-medium">
                  Pending manual payment receipts
                </p>
                <p className="text-xs text-muted-foreground">
                  Review uploaded bank / wallet receipts and approve to create
                  enrollments.
                </p>
              </div>
            ...
            {proofsError && (
              <p className="text-xs text-destructive">
                Failed to load pending receipts. Please try again.
              </p>
            )}
                  ...
                  >
                    <div className="space-y-1 text-xs">
                      ...
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        <span className="text-muted-foreground">
                          {proof.amount != null
                            ? `${proof.currency ?? "ETB"} ${proof.amount.toLocaleString()}`
                            : "–"}
                        </span>
                      </div>
                      <div className="flex flex-wrap items-center gap-2">
                ...
                <tbody>
                  {filtered.map((t) => (
                    ...
                    >
                      ...
                      <td
                        className="p-4 font-medium text-muted-foreground max-w-[140px] truncate"
                        title={formatStudentName(t.student)}
                      >
                        {formatStudentName(t.student)}
                      </td>
                      <td
                        className="p-4 text-muted-foreground text-xs max-w-[180px] truncate"
                        title={t.course?.title || (t.order ? "Cart" : "")}
                      >
                        {t.course?.title || (t.order ? "Cart" : "–")}
                      </td>
                      <td className="p-4">
                        <Badge variant="outline" className="text-xs">
                          {t.gateway || "–"}
                      ...
                      <td className="p-4 text-right font-medium">
                        ETB {t.amount != null ? t.amount.toLocaleString() : "–"}
                      </td>
                      <td className="p-4 text-right text-muted-foreground">
                        ETB{" "}
                        {t.platformShare != null
                          ? t.platformShare.toLocaleString()
                          : "–"}
                      </td>
                      <td className="p-4">
                        <Badge
        ...
        <Dialog
          open={!!previewProofId}
          onOpenChange={(open) => !open && setPreviewProofId(null)}
        >
