...
const Checkout = () => {
  ...
  const [receiptPreviewMime, setReceiptPreviewMime] = useState<string | null>(
    null,
  );
  ...
  const useBalanceMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({
        title: "Enrolled with referral balance!",
  ...
  const manualSubmitMutation = useMutation({
    mutationFn: async () => {
      if (!isStudent)
        throw new Error("Only student accounts can enroll in courses.");
      if (myEnrollmentQuery.data?.id)
        throw new Error("You are already enrolled in this course.");
      if (!course?.id) throw new Error("Course not found");
    ...
    onSuccess: () => {
      toast({
        ...
        description:
          "Your receipt is pending admin review. You will be enrolled after approval.",
      });
      setReceiptFile(null);
      ...
      <div className="min-h-screen flex flex-col bg-background">
        ...
        <main className="flex-1 container py-16">
          <p className="text-muted-foreground">
            Please sign in as a student to enroll.
          </p>
          <Button
            variant="outline"
            className="mt-4"
                          ...
                          {receiptPreviewUrl &&
                            (receiptPreviewMime?.includes("pdf") ? (
                              <iframe
                                ...
                                className="w-full h-40 rounded border bg-background"
                              />
                            ) : (
                              <img
                                src={receiptPreviewUrl}
